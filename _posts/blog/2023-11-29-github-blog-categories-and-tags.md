...
// 위 코드를 보면 `site.posts` 변수의 값을 `postlist.html`로 전달하지 않는 것을 알 수 있다. `include`로 변수를 전달하면 오류가 발생하는 문제로 고민한 결과 **`post`** 변수를 미리 선언하고 그 변수를 `postlist.html`에서 사용하는 방식으로 만들었다.
