...
파이썬에서 GUI를 만드는 라이브러리로는 **tkinter**, **pyQT**, **wxPython** 등이 대표적인데 이 중 wxPython이 스크린리더 접근성이 가장 좋다고 한다. NVDA 역시 이 wxPython을 사용한다.

wxPython을 공부하려니 [wxPython API 문서](https://docs.wxpython.org/index.html){:target="_blank"}를 제공하지만 영어로만 되어 있고, 기본적인 설명만 제공해 실제로 어떻게 코드를 작성해야 하는지 이해하기 어려운 부분이 많다.
